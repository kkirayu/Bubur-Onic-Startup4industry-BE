...
<head>
    ...
    <script>
        localStorage.setItem('token', '{{ auth()->user()->createToken("auth_token",['*'],now()->addHours(12))->plainTextToken }}')
        localStorage.setItem('crudMenu', JSON.stringify({!! json_encode(\Alurkerja\Ui\UIHelper::getAllCrudMenu()) !!}))
        localStorage.setItem('bpmnMenu', JSON.stringify({!! json_encode(\Alurkerja\Ui\UIHelper::getAllBpmnMenu()) !!}))
    </script>
